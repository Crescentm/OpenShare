...
用于 OpenWHU 进行在线资料共享，会积极合入主线更改，但同时会做一些与主线不同的更改。如有什么好的建议可以issue评论。

相对 upstream/main，目前这个分支包含这些修改：

- 面向 OpenWHU 课程资料共享场景重新组织了公开首页、文件详情、上传、反馈和下载体验。
- 增加公告、热门下载、资料上新、目录说明和文件详情等公开信息模块。
- 增加文本、Markdown、Office 和浏览器原生文件预览，并对路由和预览组件做懒加载。
- 引入 Meilisearch 作为资源搜索引擎，支持后台重建索引和公开搜索结果展示。
- 增加可在后台管理页维护的搜索语义配置，用于课程分类、资料类型别名、忽略规则和自定义分词。
- 增加本地目录托管、重新扫描和后台 worker 同步机制，用于持续共享服务器上的资料目录。
- 增加上传审核、反馈处理、回执查询和资源删除申请等公开共建流程。
- 强化超级管理员后台，支持管理员管理、系统策略、搜索索引、托管目录和审计日志管理。
- 将后端按业务域拆分为更清晰的包结构，并提取分页、路径校验、日志和字符串等通用工具。
- 优化性能，包括 SQLite 并发读取、轻量内存缓存、分页查询、路径查询去 N+1 和前端 bundle 分包。
- 将配置体系从 JSON 改为 TOML/YAML，并让业务搜索配置通过前端后台完成。
- 增加 Docker 构建、Compose 部署和本地测试脚本，并将数据目录与导入目录分开挂载。
