...
<p align="center">
面向校园、实验室、社团、研究组等中小型组织的资料管理、共享平台。
</p>

## 项目简介

OpenShare 是一个面向中小型组织内网环境的资料分享平台，适用于校园课程资料、实验室文档、社团资源、组内文件等场景。本项目部署简单，依赖少，具备完整的资料共建、资料治理和资料分发能力。

项目聚焦本地资料管理与分发，借鉴了 `OpenList`、`FileBrowser` 等项目在文件浏览和目录管理上的思路，并针对“内网资料分享”场景进行了若干功能优化，让内网环境下资料的共建&共享真正成为可能！
...
- 管理员分超级管理员和普通管理员，超级管理员初始密码在项目启动时自动生成。通过标示ID和密码进入管理后台
- 管理后台有控制台、审核、公告、日志、账号设置等页面
- 支持修改账号信息（包括头像、用户名、密码）
(以下为超级管理员特有权限)
- 访客策略配置、上传限制设置、本地目录导入
