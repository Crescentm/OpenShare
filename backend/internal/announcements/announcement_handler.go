...
func (h *AnnouncementHandler) Create(ctx *gin.Context) {
	...
	item, err := h.service.Create(ctx.Request.Context(), SaveAnnouncementInput{
		Title:      req.Title,
		Content:    req.Content,
		Status:     req.Status,
	...
	if err != nil {
		...
		if errors.Is(err, ErrAnnouncementPinDenied) {
...
func (h *AnnouncementHandler) Update(ctx *gin.Context) {
	...
	item, err := h.service.Update(ctx.Request.Context(), ctx.Param("announcementID"), SaveAnnouncementInput{
		Title:      req.Title,
		Content:    req.Content,
		Status:     req.Status,
		IsPinned:   req.IsPinned,
	...
	if err != nil {
		switch {
		case errors.Is(err, ErrAnnouncementInvalidInput):
			ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid announcement"})
		case errors.Is(err, ErrAnnouncementPinDenied):
			ctx.JSON(http.StatusForbidden, gin.H{"error": "only super admin can pin announcements"})
		case errors.Is(err, ErrAnnouncementUpdateDenied):
...
func (h *AnnouncementHandler) Delete(ctx *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, ErrAnnouncementNotFound):
			ctx.JSON(http.StatusNotFound, gin.H{"error": "announcement not found"})
		case errors.Is(err, ErrAnnouncementDeleteDenied):
