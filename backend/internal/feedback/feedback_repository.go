...
func (r *FeedbackRepository) FindFilePathByID(ctx context.Context, fileID string) (string, error) {
	...
	folderPath, err := resources.BuildFolderDisplayPath(ctx, r.db, row.FolderID)
	if err != nil {
		return "", err
	}
	return resources.NormalizeRelativePathForStorage(filepath.ToSlash(filepath.Join(folderPath, row.Name))), nil
}
