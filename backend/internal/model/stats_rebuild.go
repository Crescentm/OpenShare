...
func RebuildDashboardStatsTx(tx *gorm.DB) error {
	...
	if err := tx.Exec(`
		INSERT INTO system_stats (
			key, total_visits, total_files, total_downloads, pending_submissions, pending_feedbacks, created_at, updated_at
		)
		SELECT
			?,
			...
			COALESCE((SELECT COUNT(*) FROM submissions WHERE status = ?), 0),
				COALESCE((SELECT COUNT(*) FROM feedbacks WHERE status = ?), 0),
			`+nowExpr+`,
			`+nowExpr+`
	`, GlobalSystemStatsKey, SubmissionStatusPending, FeedbackStatusPending).Error; err != nil {
		return err
	}
