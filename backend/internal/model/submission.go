...
type Submission struct {
	...
	ReceiptCode  string         `gorm:"type:varchar(50);uniqueIndex;not null" json:"receipt_code"`       // 回执码，全局唯一
	...
	FolderID     *uuid.UUID     `gorm:"type:uuid;index" json:"folder_id,omitempty"`                      // 目标文件夹
	...
	FileID       *uuid.UUID     `gorm:"type:uuid;index" json:"file_id,omitempty"`                        // 审核通过后关联的文件ID
	UploadIP     string         `gorm:"type:varchar(45);index" json:"-"`                                 // 上传者IP，索引用于审计
