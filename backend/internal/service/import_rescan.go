...
type managedRescanIndex struct {
	foldersByPath          map[string]repository.ManagedSubtreeFolderRow
	childFoldersByParentID map[string]map[string]repository.ManagedSubtreeFolderRow
	childFolderIDsByParent map[string][]string
	filesByFolderID        map[string]map[string]repository.ManagedSubtreeFileRow
}
...
func (s *ImportService) rescanManagedDirectory(
	...
	var scanDirectory func(currentPath string, parentID *string, existing *repository.ManagedSubtreeFolderRow, forceHere bool) (string, error)
	scanDirectory = func(
		currentPath string,
		parentID *string,
		existing *repository.ManagedSubtreeFolderRow,
		forceHere bool,
	) (string, error) {
		if err := ctx.Err(); err != nil {
			return "", err
...
func buildManagedRescanIndex(
	folders []repository.ManagedSubtreeFolderRow,
	files []repository.ManagedSubtreeFileRow,
) managedRescanIndex {
	index := managedRescanIndex{
		foldersByPath:          make(map[string]repository.ManagedSubtreeFolderRow, len(folders)),
		childFoldersByParentID: make(map[string]map[string]repository.ManagedSubtreeFolderRow),
		childFolderIDsByParent: make(map[string][]string),
		filesByFolderID:        make(map[string]map[string]repository.ManagedSubtreeFileRow),
	}
	...
	for _, folder := range folders {
		...
		if folder.ParentID != nil {
			...
			if _, ok := index.childFoldersByParentID[parentID]; !ok {
				index.childFoldersByParentID[parentID] = make(map[string]repository.ManagedSubtreeFolderRow)
			}
	...
	for _, file := range files {
		...
		if _, ok := index.filesByFolderID[folderID]; !ok {
			index.filesByFolderID[folderID] = make(map[string]repository.ManagedSubtreeFileRow)
		}
...
func cloneManagedChildFolderMap(
	source map[string]repository.ManagedSubtreeFolderRow,
) map[string]repository.ManagedSubtreeFolderRow {
	if len(source) == 0 {
		return map[string]repository.ManagedSubtreeFolderRow{}
	}
	cloned := make(map[string]repository.ManagedSubtreeFolderRow, len(source))
...
func cloneManagedChildFileMap(
	source map[string]repository.ManagedSubtreeFileRow,
) map[string]repository.ManagedSubtreeFileRow {
	if len(source) == 0 {
		return map[string]repository.ManagedSubtreeFileRow{}
	}
	cloned := make(map[string]repository.ManagedSubtreeFileRow, len(source))
	for key, value := range source {
		cloned[key] = value
	}
	return cloned
