...
func (r *ImportRepository) ApplyRescanSync(ctx context.Context, input RescanSyncInput) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		for _, folder := range sortManagedFoldersForInsert(input.AddedFolders) {
...
func sortManagedFoldersForInsert(folders []*model.Folder) []*model.Folder {
	if len(folders) < 2 {
		return folders
	}
	...
	sort.SliceStable(sorted, func(i, j int) bool {
		left := managedFolderInsertDepth(sorted[i])
		right := managedFolderInsertDepth(sorted[j])
		if left != right {
			return left < right
		}
...
func managedFolderInsertDepth(folder *model.Folder) int {
	if folder == nil {
		return 0
	}
	...
	clean := filepath.Clean(strings.TrimSpace(*folder.SourcePath))
