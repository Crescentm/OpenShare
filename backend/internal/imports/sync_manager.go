...
type ImportSyncManagerOption func(*ImportSyncManager)
...
func NewImportSyncManager(importService *ImportService, options ...ImportSyncManagerOption) *ImportSyncManager {
...
func WithImportSyncRefreshInterval(interval time.Duration) ImportSyncManagerOption {
	return func(manager *ImportSyncManager) {
		if interval > 0 {
...
func WithImportSyncAuditInterval(interval time.Duration) ImportSyncManagerOption {
	return func(manager *ImportSyncManager) {
		if interval > 0 {
			manager.auditInterval = interval
		}
	}
}
