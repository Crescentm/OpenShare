...
func TestRescanManagedRootsSyncsManagedDirectoriesAndContinuesOnUnavailableRoot(t *testing.T) {
	...
	importService := NewImportService(NewImportRepository(db), storageService)
...
func TestRescanManagedDirectoryAddsNewNestedFoldersInParentFirstOrder(t *testing.T) {
	_, db, storageService := newUploadTestDeps(t)
	importService := NewImportService(NewImportRepository(db), storageService)

	rootPath := createManagedRootFixture(t, "rescan-new-folders-root")
	if _, err := importService.ImportLocalDirectory(context.Background(), LocalImportInput{RootPath: rootPath}); err != nil {
		t.Fatalf("import managed root failed: %v", err)
	}
	...
	newNestedDir := filepath.Join(rootPath, "later", "deeper")
	if err := os.MkdirAll(newNestedDir, 0o755); err != nil {
		t.Fatalf("create nested folders failed: %v", err)
	}
	if err := os.WriteFile(filepath.Join(newNestedDir, "notes.txt"), []byte("hello"), 0o644); err != nil {
		t.Fatalf("write nested file failed: %v", err)
	}

	result, err := importService.RescanManagedDirectory(context.Background(), rootFolder.ID, "", "")
	if err != nil {
		t.Fatalf("rescan managed directory failed: %v", err)
	}
	if result.AddedFolders != 2 {
		t.Fatalf("expected 2 new folders, got %+v", result)
	}
	if result.AddedFiles != 1 {
		t.Fatalf("expected 1 new file, got %+v", result)
	}

	var laterFolder model.Folder
	laterPath := filepath.Join(rootPath, "later")
	if err := db.Where("source_path = ?", laterPath).Take(&laterFolder).Error; err != nil {
		t.Fatalf("expected later folder persisted, got %v", err)
	}

	var deeperFolder model.Folder
	if err := db.Where("source_path = ?", newNestedDir).Take(&deeperFolder).Error; err != nil {
		t.Fatalf("expected deeper folder persisted, got %v", err)
	}
	if deeperFolder.ParentID == nil || *deeperFolder.ParentID != laterFolder.ID {
		t.Fatalf("expected deeper folder parent %q, got %+v", laterFolder.ID, deeperFolder.ParentID)
	}
}
