...
func NewPublicUploadService(
	...
	systemSettingService *settings.SystemSettingService,
) *PublicUploadService {
...
func (s *PublicUploadService) publishDirectUploadBatch(
	...
	items := make([]ApprovedUploadBatchItem, 0, len(submissions))
	names := make([]string, 0, len(submissions))
	rootSourcePath := strings.TrimSpace(*rootFolder.SourcePath)

	for _, submission := range submissions {
		relativeDir := resources.NormalizeRelativePathForStorage(filepath.ToSlash(filepath.Dir(submission.RelativePath)))
		...
		items = append(items, ApprovedUploadBatchItem{
