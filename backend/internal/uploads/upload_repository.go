...
func (r *UploadRepository) CreateApprovedUploadBatch(
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		for i := range submissions {
			...
			relativeDir := resources.NormalizeRelativePathForStorage(filepath.ToSlash(filepath.Dir(item.FinalRelativePath)))
			if relativeDir != "" {
				leaf, err := resources.EnsureManagedFolderPathTx(tx, rootFolder, relativeDir, reviewedAt)
				if err != nil {
