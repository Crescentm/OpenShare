...
func (s *PublicUploadService) resolveUploadFileNames(
	...
	for _, pendingPath := range pendingPaths {
		relativeToRoot := resources.NormalizeStoredSubmissionRelativePath(rootFolderDisplayPath, pendingPath)
		if relativeToRoot == "" {
			continue
		}
...
func (s *PublicUploadService) seedReservedNamesFromDisk(
	...
	relativeDir = resources.NormalizeRelativePathForStorage(relativeDir)
...
func registerExistingUploadPath(entriesByDir map[string]map[string]uploadNamespaceEntryKind, relativePath string) {
	relativePath = resources.NormalizeRelativePathForStorage(relativePath)
	if relativePath == "" {
		return
	}
