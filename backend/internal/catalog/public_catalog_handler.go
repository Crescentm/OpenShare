...
func NewPublicCatalogHandler(service *PublicCatalogService) *PublicCatalogHandler {
...
func (h *PublicCatalogHandler) ListPublicFolderFiles(ctx *gin.Context) {
	...
	result, err := h.service.ListPublicFolderFiles(ctx.Request.Context(), PublicFolderFileListInput{
	...
	if err != nil {
		switch {
		case errors.Is(err, ErrInvalidPublicFileQuery):
